...
После слияния веток `feature-1` и `feature-2` базовый код системы включает:

- модель компонента схемы `CircuitComponent`, которая описывает элемент САПР по идентификатору, типу, подписи и координатам;
- модель соединения `CircuitConnection`, которая хранит связь между двумя компонентами схемы и имя сигнала.
